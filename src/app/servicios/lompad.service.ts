...
export class LompadService{    
  ...
  async getobject(){  
    ...
    const response= await fetch("http://34.133.222.217:8000/private/read_file/?hashed_code="+this.hash+"&profile="+this.perfil, {
      method: 'GET',
      redirect: 'follow'
    });
  ...
  getObject1():Observable<any>{
    return this.http.get<any>("http://34.133.222.217:8000/private/read_file/?hashed_code="+this.hash+"&profile="+this.perfil);
  }
    
  setArchivo(data:any){
    console.log("Subiendo archivo...");    
    this.http.post("http://34.133.222.217:8000/uploadfile", data).subscribe(    
